...
class StudentService {
    ...
    static async getStudents(schoolId, params) {
        ...
        const totalCount = await db_1.prisma.student.count({ where });
    ...
    static async getStudentById(schoolId, studentId) {
        ...
        return (0, response_1.successResponse)(this.mapToResponse(student));
    }
    static async createStudent(schoolId, request) {
        ...
        if (activeCount >= maxStudents) {
            return (0, response_1.failResponse)(`Student limit reached. Your ${school.subscriptionPlan} plan allows max ${maxStudents} students. Upgrade your plan.`);
        }
        let admNo = request.admissionNumber;
        if (!admNo || !admNo.trim()) {
            const count = await db_1.prisma.student.count({ where: { schoolId } });
            const year = new Date().getFullYear();
            const seq = String(count + 1).padStart(4, "0");
            admNo = `ADM/${year}/${seq}`;
        }
        else {
            const existingAdm = await db_1.prisma.student.findFirst({
                where: { schoolId, admissionNumber: admNo },
            });
            if (existingAdm) {
                return (0, response_1.failResponse)("Admission number already exists in this school.");
            }
        }
        ...
        const hashedPassword = await (0, bcrypt_1.hashPassword)("Student@123!");
        ...
        return (0, response_1.successResponse)(this.mapToResponse(student), "Student created successfully.");
    }
    static async updateStudent(schoolId, studentId, request) {
        const student = await db_1.prisma.student.findFirst({
            where: { id: studentId, schoolId },
            include: { user: true },
        });
        ...
        return (0, response_1.successResponse)(this.mapToResponse(updatedStudent), "Student updated successfully.");
    }
    static async deleteStudent(schoolId, studentId) {
        ...
        return (0, response_1.successResponse)(true, "Student archived successfully.");
    }
    static async searchStudents(schoolId, query) {
